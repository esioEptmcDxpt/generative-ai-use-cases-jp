...
マークダウン形式で、<schema> のXMLタグで囲まれた以下の構造に従って議事録を作成してください：
        ...
        items: [
          {
            title: '創造的なアイデア生成',
            systemContext: `以下はユーザーと AI の会話です。
ユーザーは <topic></topic> の xml タグに囲まれたトピックと、<constraints></constraints> の xml タグに囲まれた制約条件を与えます。
AI はこのトピックに関する革新的で実用的なアイデアを5つ生成してください。
各アイデアは簡潔な見出しと、そのアイデアを説明する2〜3文の説明を含めてください。
AI の出力は <o> タグで始め、</o> タグで終えてください。`,
            prompt: `<topic>在宅勤務の生産性向上</topic>
<constraints>低コストで実現可能で、特別な機器を必要としないこと</constraints>`,
          },
          {
            title: 'シナリオ分析',
            systemContext: `以下はユーザーと AI の会話です。
ユーザーは <situation></situation> の xml タグに囲まれた状況と、<goal></goal> の xml タグに囲まれた目標を与えます。
AI はこの状況と目標に基づいて、考えられる3つのシナリオを分析してください。
各シナリオは「最良のケース」「最悪のケース」「最も可能性の高いケース」として、その展開と結果を説明してください。
AI の出力は <o> タグで始め、</o> タグで終えてください。`,
            prompt: `<situation>新しいサブスクリプションベースのサービスを立ち上げる予定の小規模スタートアップ</situation>
<goal>6か月以内に1000人の有料ユーザーを獲得すること</goal>`,
          },
        ],
      },
    ];
